#!/usr/bin/env node
/**
 * Apply FIRE Biz migrations to production (base + invoicing).
 *
 * Requires ONE of:
 *   - SUPABASE_DB_URL in .env.local (Postgres URI from Dashboard → Connect)
 *   - SUPABASE_ACCESS_TOKEN in .env.local (Personal Access Token) + project ref from URL
 *
 * Usage: node scripts/apply-fire-biz-production-migration.mjs
 */
...
const migrations = [
  "20260621120000_fire_biz_tables.sql",
  "20260621140000_fire_biz_invoicing_reports.sql",
];

console.log("\n--- FIRE Biz production migration apply ---\n");
...
Alternative: run combined SQL in Supabase SQL Editor:
  scripts/fire-biz-production-migration-combined.sql
`);
process.exit(1);
