-- =============================================================================
-- PRODUCTION APPLY: NEPSE Hub Admin overrides (paste ALL into Supabase SQL Editor)
-- Project: mnxxcewvgnohsavojdzu (https://mnxxcewvgnohsavojdzu.supabase.co)
-- Source: supabase/migrations/20260728030400_nepse_hub_admin_overrides_ensure.sql
-- Idempotent. Safe if partially applied. Includes PostgREST schema reload.
-- After Run: confirm verification query rows all show ok = true.
-- Then smoke-test Save Override / Restore Official Data at /nepse-hub-admin
-- =============================================================================
...
-- =============================================================================
-- POST-APPLY VERIFICATION (run in same session or as a second query)
-- Expect: every check_name shows ok = true
-- =============================================================================

select 'table_overrides_exists' as check_name,
  to_regclass('public.nepse_hub_admin_overrides') is not null as ok
union all
select 'table_audit_exists',
  to_regclass('public.nepse_hub_admin_audit_log') is not null
union all
select 'overrides_rls_enabled',
  exists (
    select 1 from pg_class c
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'nepse_hub_admin_overrides' and c.relrowsecurity
  )
union all
select 'audit_rls_enabled',
  exists (
    select 1 from pg_class c
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'nepse_hub_admin_audit_log' and c.relrowsecurity
  )
union all
...
union all
select 'fk_overrides_updated_by',
  exists (
    select 1 from pg_constraint
    where conname = 'nepse_hub_admin_overrides_updated_by_fkey'
      and conrelid = 'public.nepse_hub_admin_overrides'::regclass
...
union all
select 'idx_overrides_symbol',
  exists (select 1 from pg_indexes where schemaname='public' and indexname='nepse_hub_admin_overrides_symbol_idx')
union all
select 'idx_overrides_updated_at',
  exists (select 1 from pg_indexes where schemaname='public' and indexname='nepse_hub_admin_overrides_updated_at_idx')
union all
select 'idx_overrides_updated_by',
  exists (select 1 from pg_indexes where schemaname='public' and indexname='nepse_hub_admin_overrides_updated_by_idx')
union all
select 'idx_audit_symbol',
  exists (select 1 from pg_indexes where schemaname='public' and indexname='nepse_hub_admin_audit_log_symbol_idx')
union all
select 'idx_audit_actor',
  exists (select 1 from pg_indexes where schemaname='public' and indexname='nepse_hub_admin_audit_log_actor_idx')
union all
select 'policy_overrides_select',
  exists (select 1 from pg_policies where schemaname='public' and tablename='nepse_hub_admin_overrides' and policyname='nepse_hub_admin_overrides_service_select')
union all
select 'policy_overrides_insert',
  exists (select 1 from pg_policies where schemaname='public' and tablename='nepse_hub_admin_overrides' and policyname='nepse_hub_admin_overrides_service_insert')
union all
select 'policy_overrides_update',
  exists (select 1 from pg_policies where schemaname='public' and tablename='nepse_hub_admin_overrides' and policyname='nepse_hub_admin_overrides_service_update')
union all
select 'policy_overrides_delete',
  exists (select 1 from pg_policies where schemaname='public' and tablename='nepse_hub_admin_overrides' and policyname='nepse_hub_admin_overrides_service_delete')
union all
select 'policy_audit_select',
  exists (select 1 from pg_policies where schemaname='public' and tablename='nepse_hub_admin_audit_log' and policyname='nepse_hub_admin_audit_log_service_select')
union all
select 'policy_audit_insert',
  exists (select 1 from pg_policies where schemaname='public' and tablename='nepse_hub_admin_audit_log' and policyname='nepse_hub_admin_audit_log_service_insert')
union all
select 'grant_overrides_service_role',
  has_table_privilege('service_role', 'public.nepse_hub_admin_overrides', 'SELECT')
  and has_table_privilege('service_role', 'public.nepse_hub_admin_overrides', 'INSERT')
  and has_table_privilege('service_role', 'public.nepse_hub_admin_overrides', 'UPDATE')
  and has_table_privilege('service_role', 'public.nepse_hub_admin_overrides', 'DELETE')
union all
select 'grant_audit_service_role',
  has_table_privilege('service_role', 'public.nepse_hub_admin_audit_log', 'SELECT')
  and has_table_privilege('service_role', 'public.nepse_hub_admin_audit_log', 'INSERT')
union all
select 'timestamps_overrides',
  exists (select 1 from information_schema.columns where table_schema='public' and table_name='nepse_hub_admin_overrides' and column_name='created_at')
  and exists (select 1 from information_schema.columns where table_schema='public' and table_name='nepse_hub_admin_overrides' and column_name='updated_at');
