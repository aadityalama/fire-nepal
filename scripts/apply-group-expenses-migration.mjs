#!/usr/bin/env node
/**
 * Apply the Group Expenses production migration only.
 *
 * Requires ONE of:
 ...
 *
 * Alternative: run this self-contained SQL in Supabase SQL Editor:
 *   scripts/group-expenses-production-migration-combined.sql
 */
...
const migrationFile = "20260711120000_group_expenses.sql";
...
async function applyWithPg(dbUrl) {
  ...
  for (const url of attempts) {
    ...
    try {
      ...
      await client.end();
      console.log("\nOK: Group Expenses migration SQL applied via pg.\n");
      return true;
    } catch (e) {
      lastErr = e;
      try {
  ...
  console.error(`\nFAIL: pg could not apply Group Expenses migration - ${lastErr?.message ?? "unknown error"}\n`);
  return false;
}
...
async function applyWithManagementApi(accessToken, projectRef) {
  ...
  console.log("\nOK: Group Expenses migration applied via Management API.\n");
  return true;
}
...
console.log("\n--- Group Expenses production migration apply ---\n");
...
console.error(`
FAIL: Cannot apply Group Expenses migration - set ONE of these in .env.local:
