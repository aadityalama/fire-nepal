-- FIRE Nepal: Ensure finance_budget_records is fully provisioned in production.
...
-- 2. Add soft-delete column if missing
...
-- Verification query (run separately after the above):
-- select
--   table_name,
--   (select count(*) from information_schema.columns c where c.table_name = 'finance_budget_records' and c.table_schema = 'public') as column_count,
--   (select count(*) from pg_policies p where p.tablename = 'finance_budget_records') as policy_count
-- from information_schema.tables
-- where table_schema = 'public' and table_name = 'finance_budget_records';
