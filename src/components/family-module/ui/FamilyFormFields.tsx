...
export function FamilyInput({
  ...
  return (
    <input
      className={`w-full rounded-xl border px-3 py-2.5 text-sm font-semibold outline-none transition focus:ring-2 focus:ring-emerald-500/40 disabled:cursor-not-allowed disabled:opacity-60 ${
        light
...
export function FamilyTextarea({
  ...
  return (
    <textarea
      className={`min-h-[100px] w-full resize-y rounded-xl border px-3 py-2.5 text-sm font-semibold outline-none transition focus:ring-2 focus:ring-emerald-500/40 disabled:cursor-not-allowed disabled:opacity-60 ${
        light
          ? "border-emerald-200/80 bg-white text-slate-900 placeholder:text-slate-400"
          : "border-white/10 bg-black/30 text-white placeholder:text-zinc-500"
      } ${className}`}
