...
export function FireAiExpenseInsights() {
  const light = useFireTheme().resolvedTheme === "light";
  const { expenseInsights, hydrated } = useFireAiData();
  ...
  if (!expenseInsights.hasData) {
    return (
      <FireAiGlassCard className="text-center">
        ...
        <h2 className={`mt-4 text-lg font-black ${light ? "text-slate-900" : "text-white"}`}>No expense data yet</h2>
        <p className={`mt-2 text-sm font-medium leading-relaxed ${light ? "text-slate-500" : "text-emerald-200/60"}`}>
          Add expenses in Expense Dashboard or set up your Cashflow to unlock spending insights.
        </p>
        <Link
  ...
  return (
    <div className="space-y-3">
      {expenseInsights.metrics.map((metric) => (
        <FireAiGlassCard key={metric.id}>
