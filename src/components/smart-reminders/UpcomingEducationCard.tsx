...
import { isEducationReminderType, reminderPriority } from "@/lib/smart-reminders/reminder-engine";
...
export function UpcomingEducationCard({
  reminders,
  upcomingWithinDays,
  onMarkPaid,
}: {
  ...
  const now = new Date();
  const rows = useMemo(() => {
    return reminders.filter((r) => isEducationReminderType(r.reminderType)).sort(sortByDue).slice(0, 6);
  }, [reminders]);
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute -right-16 top-10 h-48 w-48 rounded-full bg-gradient-to-bl from-sky-400/14 via-emerald-400/10 to-transparent blur-3xl dark:from-sky-400/10 dark:via-emerald-400/8"
      />
      <div className="relative">
        <p className="text-[11px] font-black uppercase tracking-[0.16em] text-emerald-700/90 dark:text-emerald-300/75">Kids & learning</p>
        <h3 className="mt-1 text-lg font-black tracking-tight text-slate-900 dark:text-white">Upcoming education fees</h3>
        <p className="mt-1 text-xs font-semibold text-slate-600 dark:text-zinc-400">School fees, tuition, and exam milestones</p>
      </div>
      ...
      <div className="relative mt-4 space-y-2">
        {rows.length ? (
          ...
          <p className={`text-sm font-semibold ${light ? "text-slate-600" : "text-zinc-400"}`}>
            No education reminders yet — add school fees or exam dates.
          </p>
        )}
      </div>
    </section>
