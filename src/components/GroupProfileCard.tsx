  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="relative z-[71] flex max-h-[min(92vh,720px)] w-full max-w-lg flex-col overflow-hidden rounded-t-[1.5rem] border border-[#E5E7EB] bg-white shadow-2xl sm:rounded-[1.5rem]"
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-[#E5E7EB] px-4 pb-3 pt-4 sm:px-5">
          <div className="min-w-0">
            <h2 id={titleId} className="text-lg font-black text-[#111827]">
              {title}
            </h2>
            {subtitle ? (
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-[#E5E7EB] text-[#6B7280] transition hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
...
function GroupLogo({ logoUrl = "", companyName, size = "md" }: GroupLogoProps) {
  ...
  return (
    <div
      className={`grid ${sizeClass} shrink-0 place-items-center overflow-hidden border border-[#E5E7EB] bg-white font-black text-[#10B981] shadow-sm`}
      aria-hidden
    >
      {showImage ? (
  ...
  return (
    <>
      <motion.article
        ...
        className="rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm sm:p-[1.125rem]"
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1 pr-1">
            {branded ? (
              <>
                {profile.companyName ? (
                  <h2 className="truncate text-[17px] font-black leading-tight tracking-tight text-[#111827] sm:text-lg">
                    {profile.companyName}
                  </h2>
                ) : null}
                {roomLabel ? (
                  <p className="truncate text-sm font-semibold text-[#10B981]">{roomLabel}</p>
                ) : null}
                {profile.companyType ? (
                ...
                {profile.description ? (
                  <p className="mt-1.5 line-clamp-2 text-xs font-medium leading-relaxed text-[#6B7280]">
                    {profile.description}
                  </p>
                ) : null}
              </>
              ...
              <>
                <h2 className="text-[17px] font-black tracking-tight text-[#111827] sm:text-lg">
                  Group Profile
                </h2>
                <p className="mt-1 text-xs font-medium leading-relaxed text-[#6B7280]">
                  Add company or room details for branded settlement reports.
                </p>
              </>
            )}
            <span
              className="mt-2.5 inline-flex min-h-6 items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-emerald-700 ring-1 ring-emerald-200"
          ...
          <button
            ...
            className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-700 transition hover:bg-emerald-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 active:scale-95"
      ...
      >
        <div className="space-y-3 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2 sm:px-5">
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-3.5">
            ...
            <div className="mt-2.5 flex min-w-0 items-center gap-3">
              <div className="relative">
                ...
                {logoUploading ? (
                  <span className="absolute inset-0 grid place-items-center rounded-2xl bg-white/80">
                    <Loader2 className="animate-spin text-emerald-600" size={20} />
                  </span>
                ) : null}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-[#111827]">Group logo / photo</p>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <button
                    ...
                    className="inline-flex min-h-11 items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px] font-black text-emerald-700 transition hover:bg-emerald-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 disabled:opacity-60"
                  >
                    <Camera size={13} />
                  ...
                  {draft.logoUrl ? (
                    <button
                      ...
                      className="inline-flex min-h-11 items-center rounded-xl px-3 py-2 text-[11px] font-bold text-red-600 transition hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300"
                    >
                      Remove
                    </button>
                  ) : null}
          ...
          <label className="block">
            <span className="mb-1 block text-[10px] font-black uppercase tracking-wide text-[#6B7280]">
              Company / group name
            </span>
            <input
              value={draft.companyName}
              onChange={(event) => setDraft((current) => ({ ...current, companyName: event.target.value }))}
              className="w-full min-w-0 rounded-xl border border-[#E5E7EB] bg-white px-3 py-2.5 text-sm font-bold text-[#111827] outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
          ...
          <label className="block">
            ...
            <textarea
              ...
              className="min-h-20 w-full min-w-0 resize-none rounded-xl border border-[#E5E7EB] bg-white px-3 py-2.5 text-sm font-bold text-[#111827] outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
              placeholder="Shared apartment for KP Electric workers"
            />
          </label>

          {draftBranded ? (
            <div className="rounded-2xl border border-dashed border-[#E5E7EB] bg-gray-50 px-3 py-3">
              <p className="text-[10px] font-black uppercase tracking-wide text-[#6B7280]">
                Settlement header preview
              </p>
              <div className="mt-2 rounded-xl border border-[#E5E7EB] bg-white p-3">
                <SettlementBrandingHeader
                  companyName={draft.companyName.trim()}
                  roomNumber={draft.roomNumber.trim()}
                  logoUrl={draft.logoUrl.trim()}
          ...
          <button
            ...
            className="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-emerald-600 px-3 py-2.5 text-sm font-black text-white transition hover:bg-emerald-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 active:bg-emerald-800 disabled:opacity-60"
          >
            {saving ? (
              <>
