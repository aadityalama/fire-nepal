  ...
  return (
    ...
    >
      <label className="block">
        <span className="mb-0.5 block text-[10px] font-semibold uppercase text-gray-100">Amount ({line.currency})</span>
        <input
          value={amtStr}
      ...
      <div className="relative">
        <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-2">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-sky-500/30 to-emerald-500/20 text-white ring-1 ring-sky-400/25">
              <Landmark size={18} />
            </div>
            <div>
              <h2 className="text-base font-black text-white sm:text-lg">Banking & cash</h2>
              <p className="text-xs font-bold leading-snug text-gray-100 sm:text-sm">
        ...
        <div className="mb-4 grid gap-2 sm:grid-cols-3">
          <div className="rounded-2xl border border-sky-400/20 bg-gradient-to-br from-slate-950/70 to-black/40 px-3 py-3 shadow-lg shadow-black/30 ring-1 ring-white/[0.04] backdrop-blur-md">
            <p className="text-[11px] font-black uppercase tracking-wide text-white sm:text-xs">Liquid (NPR)</p>
            <p className="mt-1 text-xl font-black tabular-nums text-white sm:text-2xl">{formatMoney(totals.liquidNpr, "NPR")}</p>
            <p className="mt-1 text-[11px] font-semibold leading-snug text-gray-100 sm:text-xs">Sum of current balances</p>
          </div>
          <div className="rounded-2xl border border-amber-400/25 bg-gradient-to-br from-amber-950/50 to-black/40 px-3 py-3 shadow-lg shadow-black/30 ring-1 ring-white/[0.04] backdrop-blur-md">
            <p className="text-[11px] font-black uppercase tracking-wide text-white sm:text-xs">FD principal (NPR)</p>
            <p className="mt-1 text-xl font-black tabular-nums text-white sm:text-2xl">{formatMoney(totals.fdPrincipalNpr, "NPR")}</p>
            <p className="mt-1 text-[11px] font-semibold leading-snug text-gray-100 sm:text-xs">Locked term deposits</p>
          </div>
          <div className="rounded-2xl border border-lime-400/20 bg-gradient-to-br from-lime-950/40 to-black/40 px-3 py-3 shadow-lg shadow-black/30 ring-1 ring-white/[0.04] backdrop-blur-md">
            <p className="text-[11px] font-black uppercase tracking-wide text-white sm:text-xs">FD income (est. / mo)</p>
            <p className="mt-1 text-xl font-black tabular-nums text-white sm:text-2xl">{formatMoney(totals.fdMonthlyNpr, "NPR")}</p>
            <p className="mt-1 text-[11px] font-semibold leading-snug text-gray-100 sm:text-xs">Modelled · syncs to cashflow</p>
          </div>
        </div>
            ...
            <div className="rounded-2xl border border-sky-400/20 bg-gradient-to-br from-slate-950/65 via-black/40 to-sky-950/25 p-3 shadow-lg shadow-black/30 ring-1 ring-white/[0.05] backdrop-blur-md sm:p-4">
              <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs font-semibold leading-snug text-gray-100 sm:text-sm">
                    {editingId
              ...
              <div key={draft.id} className="space-y-2">
                <div className="grid gap-2 lg:grid-cols-12 lg:items-end lg:gap-2">
                  <label className="block min-w-0 lg:col-span-3">
                    <span className="mb-0.5 block text-[10px] font-semibold uppercase tracking-wide text-gray-100">
                      Bank / wallet name
                    </span>
                    <input
                      type="text"
                  ...
                  <div className="min-w-0 lg:col-span-3">
                    <NumericMoneyInput
                      ...
                      variant="amount"
                      placeholder="0"
                      className="text-[10px] font-semibold uppercase tracking-wide text-gray-100 [&>span]:block"
                      ...
                      inputClassName="min-w-0 flex-1 bg-transparent text-xs font-semibold text-white outline-none sm:text-sm"
                    />
                  </div>
                  <label className="block w-full lg:col-span-2">
                    <span className="mb-0.5 block text-[10px] font-semibold uppercase tracking-wide text-gray-100">Currency</span>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  ...
                  {editingId ? (
                    <button
                      type="button"
                      onClick={resetEntryForm}
                      className="rounded-xl border border-white/15 bg-white/5 px-4 py-2.5 text-xs font-semibold text-gray-100 transition hover:bg-white/10"
                    >
                      Cancel
                    </button>
                  ) : null}
            ...
            <div className="space-y-2">
              ...
              {lines.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-sky-400/25 bg-black/20 px-4 py-8 text-center">
                  <p className="text-sm font-bold text-gray-100">No bank accounts yet.</p>
                  <p className="mt-1 text-xs font-semibold text-gray-100">
                    Complete the form above and tap <span className="font-black text-white">Save</span> — it will show up
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0 flex-1 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-bold text-gray-100">
                          {line.accountNumber ? (
                            <span>
                              <span className="font-black text-gray-200">Account · </span>
                              {line.accountNumber}
                            </span>
                          ) : null}
                          <span>
                            <span className="font-black text-gray-200">Balance · </span>
                            {formatMoney(line.amount ?? 0, line.currency)}
                          </span>
                          <span className="font-black text-white">{line.currency}</span>
                        </div>
                        <div className="flex flex-col gap-1 border-t border-emerald-400/10 pt-2 text-[11px] font-semibold text-gray-100 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-4">
                          {line.openedDate ? (
                            <span>
                              <span className="font-black text-gray-200">Opened · </span>
                              {line.openedDate}
                            </span>
                          ) : (
                            <span className="text-gray-200">No opening date on file</span>
                          )}
            ...
            {fixedDeposits.map((fd) => {
              ...
              return (
                ...
                >
                  <div className="grid gap-2 lg:grid-cols-12 lg:items-end lg:gap-2">
                    <label className="block min-w-0 lg:col-span-3">
                      <span className="mb-0.5 block text-[10px] font-semibold uppercase tracking-wide text-gray-100">Bank name</span>
                      <input
                        type="text"
                        value={fd.bankName}
                    ...
                    <div className="flex justify-end lg:col-span-1 lg:items-end lg:pb-0.5">
                      <button
                        ...
                        className="rounded-xl p-2 text-gray-400 transition hover:bg-rose-500/15 hover:text-rose-300"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
                  ...
                  <div className="grid gap-2 border-t border-amber-400/15 pt-2 sm:grid-cols-2 lg:grid-cols-4 lg:items-end">
                    ...
                    <div className="rounded-xl border border-emerald-400/15 bg-black/30 px-3 py-2 sm:col-span-2 lg:col-span-1">
                      <p className="text-[10px] font-black uppercase tracking-wide text-white">Est. monthly interest</p>
                      <p className="mt-0.5 text-base font-black tabular-nums text-white sm:text-lg">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div className="rounded-xl border border-sky-400/15 bg-black/25 px-3 py-2">
                      <p className="text-[10px] font-black uppercase tracking-wide text-white">Maturity amount (est.)</p>
                      <p className="mt-0.5 text-base font-black tabular-nums text-white sm:text-lg">
                        {derived
                    ...
                    <div className="rounded-xl border border-lime-400/15 bg-black/25 px-3 py-2">
                      <p className="text-[10px] font-black uppercase tracking-wide text-white">Est. annual return</p>
                      <p className="mt-0.5 text-base font-black tabular-nums text-white sm:text-lg">
                        {derived?.estimatedAnnualReturnPct != null
                          ? `${derived.estimatedAnnualReturnPct.toFixed(2)}% effective`
                          : "—"}
                      </p>
                      <p className="mt-1 text-[10px] font-semibold text-gray-100">
