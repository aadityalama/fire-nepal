...
function PortfolioTotalsStrip() {
  ...
  const labelMuted = light ? "text-slate-800" : "text-gray-100";
  ...
  return (
    <div className="mb-5 grid gap-3 sm:mb-6 sm:grid-cols-3">
      <div className={`relative overflow-hidden rounded-2xl border px-4 py-3 backdrop-blur-xl ${cardBase}`}>
        ...
        <p className={`mt-1 truncate text-lg font-black tracking-tight sm:text-xl ${light ? "text-black" : "text-white"}`}>
      ...
      <div
        className={`relative overflow-hidden rounded-2xl border px-4 py-3 backdrop-blur-xl ${
          light ? "border-slate-200/80 bg-white/85 text-black" : "border-white/10 bg-white/[0.05] text-white"
        }`}
      >
        <div className={`text-[10px] font-black uppercase tracking-[0.14em] ${light ? "text-black" : labelMuted}`}>Total assets</div>
    ...
    <WealthDashboardShell
      ...
      footerNote={
        <>
          ...
          >
            <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-wide ${light ? "text-black" : muted}`}>
              <Cloud size={14} className={light ? "text-emerald-600" : "text-emerald-400/80"} />
              Data sync
            </div>
            <p className={`mt-1 text-[11px] font-semibold ${light ? "text-slate-800" : "text-gray-100"}`}>
              Last backup: Today, 10:45 AM
            </p>
            <p className={`mt-0.5 text-[10px] font-semibold ${light ? "text-slate-800" : "text-gray-100"}`}>
