...
const MEMBERSHIP_RENEWAL_CRON_ROW_ID = "membership_renewal_reminders_cron" as const;
...
export async function upsertMembershipRenewalRemindersCronHealth(
  sb: SupabaseClient<Database>,
  input: {
    last_status: string;
    last_run_at?: string;
  ...
  const { error } = await sb.from("system_health").upsert(
    {
      id: MEMBERSHIP_RENEWAL_CRON_ROW_ID,
      label: "Membership renewal reminder emails cron",
      last_run_at,
      last_status: input.last_status,
      metadata: (input.metadata ?? {}) as Json,
    },
