...
- **Migration** — `supabase/migrations/20250606180000_membership_payment_requests.sql` adds `public.membership_requests` and storage. **`20250607080000_membership_requests_align_columns.sql`** renames legacy columns to `plan_type`, `proof_url`, and `created_at` when needed. **`20250606220000_ensure_membership_payment_storage_bucket.sql`** re-applies bucket + policies if storage was skipped earlier.
- **Runtime** — `POST /api/membership-requests` provisions the bucket via the Storage API when missing (needs `SUPABASE_SERVICE_ROLE_KEY`) and uploads proof files with the service role so uploads work even before storage RLS is applied. Proof object paths stay `{user_id}/{request_id}.ext` in `proof_url` (private bucket; signed URLs are minted in API routes).
